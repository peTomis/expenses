...
class _TransactionsPageState extends ConsumerState<TransactionsPage> {
  ...
  void initState() {
    super.initState();
    _filter = ref.read(transactionsFilterProvider);
    if (_filter != null) {
  ...
  Widget build(BuildContext context) {
    ...
    if (_search.isNotEmpty) {
      ...
      entries = entries
          .where((e) => e.merchant.toLowerCase().contains(query))
          .toList();
    }
                                ...
                                for (final entry in dayEntries)
                                  TransactionRow(
                                    ...
                                    onTap: () => Navigator.of(context).push(
                                      MaterialPageRoute(
                                        builder: (_) =>
                                            TransactionDetailPage(entry: entry),
                                      ),
                                    ),
                                  ),
                              ],
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: selected ? color : textColor.withValues(alpha: 0.14),
          ),
        ),
        child: Row(
          mainAxisSize: MainAxisSize.min,
